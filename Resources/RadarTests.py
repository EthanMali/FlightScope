...
class TRACONDisplay(QMainWindow):
    def __init__(self, username, password, tracon_config):
        super().__init__()
    ...
    def draw_aircraft(self, painter):
        ...
        print(f"Radar center: {center_lat}, {center_lon}")  # Debug statement

        for aircraft in self.aircraft_data:
            try:
                # Safely access aircraft data
                lat = aircraft.get('lat')  # Latitude
                lon = aircraft.get('lon')  # Longitude
                alt = aircraft.get('alt')  # Altitude
                callsign = aircraft.get('flight', "N/A")  # Callsign
                speed = f"{int(aircraft.get('gs', 0))}" if aircraft.get('gs') else "N/A"  # Ground speed
                ...
                # Format altitude (e.g., 10000 -> 100, 5000 -> 050)
                ...
                # Assign sector (for sector-based display, you can add this in the next step)
